...
def _extract_complementary_data(batch: dict[str, Any]) -> dict[str, Any]:
    ...
    This includes padding flags, task description, indices, and optional
    training-only side-channel targets/weights such as `loss.*` and `aux.*`.
    ...
    loss_keys = {k: v for k, v in batch.items() if str(k).startswith("loss.")}
    aux_keys = {k: v for k, v in batch.items() if str(k).startswith("aux.")}
