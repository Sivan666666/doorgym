...
ROOT="${1:-high-level/logs/foundationpose/wc4_sam3_fp_grasp_64_seed615455575_v2}"
mkdir -p "$ROOT"
...
for seed in "${SEEDS[@]}"; do
  for offset in "${OFFSETS[@]}"; do
    ...
    mkdir -p "$run_dir"
    echo "===== SAM3 + FoundationPose seed=${seed} offset=${offset} ====="
...
print(
    f"SAM3_FOUNDATIONPOSE_GRASP_SUCCESS {success}/{trials} "
