...
def parse_args():
    args = gymutil.parse_arguments(
        description="A2W+Z1 offset-base float IK door-pull demo.",
        headless=True,
        no_graphics=True,
        custom_parameters=[
            ...
            {
                "name": "--pull_offset_right_boundary_margin",
                "type": float,
                "default": 0.04,
                "help": (
                    "Place the base center this far beyond the door's right boundary "
                    "at spawn. For the default WC4 pull view, right is world +Y."
                ),
            },
            ...
            {
                "name": "--pull_base_retreat_full_angle_deg",
                "type": float,
                "default": 45.0,
                "help": (
                    "Door angle at which the primary straight-back base retreat is complete. "
                    "This is intentionally independent of the release angle so asking the arm "
                    "to pull farther does not make the base retreat too slowly."
                ),
            },
            {
            ...
            },
            {
                ...
                "type": int,
                "default": 100,
                "help": "Frames used to smoothly apply the late pull retreat.",
            },
            {
            ...
            {
                "name": "--pull_offset_release_right_shift",
                "type": float,
                "default": 0.0,
                "help": (
                    "Legacy world +Y release shift. The pull_offset controller "
                    "defaults to zero because A2W cannot translate laterally."
                ),
            },
            {
                "name": "--pull_offset_release_right_shift_delay_fraction",
                "type": float,
                "default": 0.20,
                "help": (
                    "Delay the release-phase +Y base shift until the gripper has "
                    "completed its outward physical separation."
                ),
            },
            {
            ...
            },
            {
                ...
                "help": (
                    ...
                    "The default treats 49.5 degrees as reaching a 50-degree "
                    "release target, avoiding an indefinite contact equilibrium "
                    "within the final half degree."
                ),
            },
            {
            ...
            },
            {
                ...
                "type": int,
                "default": 0,
                "help": "Frames that keep pulling while the gripper starts loosening.",
            },
            {
                "name": "--release_handle_steps",
                "type": int,
                "default": 120,
                "help": "Frames used to open the gripper, move outward, and retract laterally.",
            },
            {
                ...
                "default": 0.04,
                "help": (
                    "Fraction of release_handle used to open the gripper after the "
                    "short outward preload."
                ),
            },
            {
                "name": "--release_gripper_open_delay_fraction",
                "type": float,
                "default": 0.48,
                "help": (
                    "Initial release fraction that keeps the gripper physically closed "
                    "during the short outward pull. This transfers momentum through "
                    "handle contact; it does not apply an auxiliary door force."
                ),
            },
            {
            ...
            {
                "name": "--pull_offset_post_release_coast_steps",
                "type": int,
                "default": 180,
                "help": (
                    "Frames spent stationary after the physical release so the unforced, "
                    "zero-resistance door can coast clear before the non-lateral direct pass."
                ),
            },
            {
                "name": "--pull_offset_pass_split",
                "type": float,
                "default": 0.68,
                "help": (
                    "Fraction of pass_through used to reach the door centerline behind "
                    "the doorway; the remainder continuously bends toward world +Y."
                ),
            },
            {
                "name": "--pull_offset_entry_tangent_distance",
                "type": float,
                "default": 0.45,
                "help": "Forward tangent length at the beginning of the center-seeking pass curve.",
            },
            {
                "name": "--pull_offset_center_tangent_distance",
                "type": float,
                "default": 0.85,
                "help": "Straight-through tangent length before reaching the post-door center point.",
            },
            {
                "name": "--pull_offset_center_control_right_fraction",
                "type": float,
                "default": 0.0,
                "help": (
                    "Fraction of the right-side release Y retained by the final "
                    "control point before merging to the post-door center."
                ),
            },
            {
                "name": "--pull_offset_center_control_lateral_offset",
                "type": float,
                "default": 0.0,
                "help": (
                    "World-Y offset added to the final pre-center control point. "
                    "Negative values center the A2W body earlier, but may reduce "
                    "clearance from the still-moving door panel."
                ),
            },
            {
                "name": "--pull_offset_exit_forward_tangent_distance",
                "type": float,
                "default": 0.30,
                "help": "Forward tangent retained just after crossing before bending toward +Y.",
            },
            {
                "name": "--pull_offset_exit_y_tangent_distance",
                "type": float,
                "default": 0.35,
                "help": "Final +Y tangent length of the moving turn after crossing.",
            },
            {
                "name": "--pull_offset_final_y_distance",
                "type": float,
                "default": 0.0,
                "help": "Deprecated compatibility option; the final +Y turn is disabled.",
            },
            {
                "name": "--pull_offset_final_forward_distance",
                "type": float,
                "default": 0.35,
                "help": (
                    "Additional straight-ahead travel after clearing the doorway. "
                    "The final base heading remains its body +X forward direction."
                ),
            },
    ...
    # This offset-pull variant must remain physically passive at the hinge:
    # opening comes only from the gripper/handle contact and base/arm motion.
    args.door_auto_open_force = 0.0
    args.door_open_resistance = 0.0
    args.wc4_disable_door_open_resistance = True
    # This pull-offset task models a freely swinging WC4 door.  Keep both the
    # explicit closing resistance and the lower-level hinge losses disabled;
    # otherwise per-env randomization can silently stop the door after release.
    # No positive force or torque is injected into the hinge.
    args.door_joint_friction = 0.0
    args.door_joint_damping = 0.0
    args.ikpush_door_joint_friction_rand = 0.0
    args.ikpush_door_joint_damping_rand = 0.0
    # The spawn lane is already the rotate-handle lane, so the entire
    # pre-pull sequence uses a fixed base yaw.
    args.move_to_approach_vyaw = 0.0
    args.move_to_approach_yaw_delta = 0.0
    args.push_base_yaw_delta = 0.0
    args._explicit_cli_flags.update(
        {
            "--door_joint_friction",
            "--door_joint_damping",
        }
...
def pull_offset_right_direction(yaw):
    """World-space right direction for a planar base heading."""
    return np.asarray(
...
def compute_pull_offset_base_walk_targets(args, door):
    """Keep spawn and rotate_handle on the same lateral lane."""
    yaw_start, heading, base_start, legacy_base_stop = dc.compute_base_walk_targets(args, door)
    right = pull_offset_right_direction(yaw_start)
    target_projection = float(np.dot(legacy_base_stop, right))
    base_start = (
        np.asarray(base_start, dtype=np.float32)
        + right * (target_projection - float(np.dot(base_start, right)))
    )
    return (
        float(yaw_start),
        heading,
        base_start.astype(np.float32),
        np.asarray(legacy_base_stop, dtype=np.float32),
    )


def pull_offset_door_center_xy(args, door):
    world_min_x, world_max_x, world_min_y, world_max_y = dc.door_world_xy_bounds_from_bbox(
        args,
        door,
    )
    return np.asarray(
        [0.5 * (world_min_x + world_max_x), 0.5 * (world_min_y + world_max_y)],
        dtype=np.float32,
    )


def compute_pull_offset_base_targets(args, door, base_stop, heading):
    """Return a post-door target that finishes straight along body +X."""
    heading = normalize(np.asarray(heading, dtype=np.float32))
    door_center = pull_offset_door_center_xy(args, door)
    post_door_center = door_center + heading * (
        float(args.robot_rear_offset) + float(args.door_pass_clearance)
    )
    final_target = post_door_center + heading * max(
        0.0,
        float(getattr(args, "pull_offset_final_forward_distance", 0.35)),
    )
    return post_door_center.astype(np.float32), final_target.astype(np.float32)
...
def cubic_bezier_point(p0, p1, p2, p3, t):
    ...
    omt = 1.0 - t
    return (
        omt * omt * omt * np.asarray(p0, dtype=np.float32)
        + 3.0 * omt * omt * t * np.asarray(p1, dtype=np.float32)
        + 3.0 * omt * t * t * np.asarray(p2, dtype=np.float32)
        + t * t * t * np.asarray(p3, dtype=np.float32)
    ).astype(np.float32)
...
def cubic_bezier_tangent(p0, p1, p2, p3, t):
    ...
    omt = 1.0 - t
    tangent = (
        3.0 * omt * omt * (np.asarray(p1, dtype=np.float32) - np.asarray(p0, dtype=np.float32))
        + 6.0 * omt * t * (np.asarray(p2, dtype=np.float32) - np.asarray(p1, dtype=np.float32))
        + 3.0 * t * t * (np.asarray(p3, dtype=np.float32) - np.asarray(p2, dtype=np.float32))
    )
...
def trajectory_targets(
    ...
    base_forward = np.asarray(
        [math.cos(float(yaw_stop)), math.sin(float(yaw_stop))],
        dtype=np.float32,
    )
    base_retreat_dir = -normalize(base_forward)
    ...
    base_pull = np.asarray(
        traj.get(
            ...
            + base_retreat_dir * float(args.pull_base_retreat_distance),
        ),
        dtype=np.float32,
    )
    base_release = np.asarray(
        traj.get(
            "base_release",
            np.asarray(base_stop, dtype=np.float32)
            + base_retreat_dir * float(args.release_base_retreat_distance),
        ),
        dtype=np.float32,
    )
    if "base_release" not in traj:
        base_release = base_release.copy()
        base_release += pull_offset_right_direction(yaw_stop) * float(
            getattr(args, "pull_offset_release_right_shift", 0.0)
        )
    ...
    post_release_coast_end = release_end + max(
        0,
        int(args.pull_offset_post_release_coast_steps),
    )
    pass_end = post_release_coast_end + max(1, int(args.pass_through_steps))
    ...
    phase = "walk"
    if (
        "home_ee_base_pos" not in traj
        and int(step) >= int(walk_end)
    ...
    if step < walk_end:
        ...
        if "offset_approach_curve" not in traj:
            start_forward = np.asarray(
                [math.cos(float(yaw_start)), math.sin(float(yaw_start))],
                dtype=np.float32,
            )
            stop_forward = np.asarray(
                [math.cos(float(yaw_stop)), math.sin(float(yaw_stop))],
                dtype=np.float32,
            )
            ...
            traj["offset_approach_curve"] = (
                np.asarray(base_start, dtype=np.float32).copy(),
                np.asarray(base_start, dtype=np.float32)
                + start_forward * tangent_distance,
                np.asarray(base_stop, dtype=np.float32)
                - stop_forward * tangent_distance,
                np.asarray(base_stop, dtype=np.float32).copy(),
            )
        approach_curve = traj["offset_approach_curve"]
        base_xy = cubic_bezier_point(*approach_curve, t)
        tangent = cubic_bezier_tangent(*approach_curve, t)
        yaw = (
            math.atan2(float(tangent[1]), float(tangent[0]))
            if float(np.linalg.norm(tangent)) > 1.0e-7
            else float(yaw_start)
        )
        target_pos = ik_state.current_pos_np.copy() if ik_state.current_pos_np is not None else pregrasp.copy()
        ...
        if step < initial_end:
            ...
            if "pull" not in traj:
                ...
                traj["base_release"] += pull_offset_right_direction(yaw_stop) * float(
                    getattr(args, "pull_offset_release_right_shift", 0.0)
                )
                traj["base_retreat_dir"] = base_retreat_dir.copy()
            ...
            if (
                ...
                print(
                    ...
                    f"release begins at step="
                    f"{traj['pull_complete_step'] + max(0, int(args.open_hold_steps))}.",
                    flush=True,
                )
            ...
            if door.open_stage:
                ...
                base_progress = np.clip(
                    ...
                    / max(
                        1.0e-6,
                        float(args.pull_base_retreat_full_angle_deg),
                    ),
                    0.0,
                    1.0,
                )
            ...
            base_xy = (
                ...
                + np.asarray(
                    traj.get("base_retreat_dir", base_retreat_dir),
                    dtype=np.float32,
                )
            ...
            if (
                ...
                and int(step) + 1 >= int(traj["pull_complete_step"])
            ...
            release_t = float(
                np.clip(
                    ...
                    / max(1, int(args.release_handle_steps)),
                    0.0,
                    1.0,
                )
            )
            if "release_start_pos" not in traj:
                ...
                if home_world is not None:
                    # home_ee_base_pos is cached only after the offset approach
                    # has merged into the legacy stop lane. It is therefore a
                    # compact, valid stow pose rather than the pre-merge pose.
                    release_retract = np.asarray(home_world, dtype=np.float32).copy()
                else:
                    release_retract = release_outer.copy()
                    release_retract[1] += float(args.release_lateral_distance)
            ...
            gripper_open_delay_fraction = float(
                np.clip(
                    getattr(args, "release_gripper_open_delay_fraction", 0.20),
                    0.0,
                    0.95,
                )
            )
            ...
            target_quat = (
                None
                if args.ik_position_only
                else traj.get("release_quat", traj.get("turned_quat"))
            )
            gripper_open_t = np.clip(
                (release_t - gripper_open_delay_fraction)
                / max(1.0e-6, gripper_open_fraction),
                0.0,
                1.0,
            )
            gripper = float(
                lerp(
                    ...
                    smoothstep(float(gripper_open_t)),
                )[0]
            )
            ...
            right_shift_delay = float(
                np.clip(
                    getattr(
                        args,
                        "pull_offset_release_right_shift_delay_fraction",
                        0.20,
                    ),
                    0.0,
                    0.95,
            ...
            right_shift_t = smoothstep(
                float(
                    np.clip(
                        (release_t - right_shift_delay)
                        / max(1.0e-6, 1.0 - right_shift_delay),
                        0.0,
                        1.0,
                    )
                )
            ...
            base_xy[1] = float(
                lerp(
                    np.asarray([release_base_start[1]], dtype=np.float32),
                    np.asarray([base_release[1]], dtype=np.float32),
                    right_shift_t,
                )[0]
            )
            yaw = yaw_stop
            phase = "release_handle"
        elif step < post_release_coast_end:
            target_pos = np.asarray(
                traj.get(
                    "release_retract_pos",
                    traj.get("last_target_pos", traj["pull"]),
            ...
            if args.ik_position_only:
                ...
                release_quat = traj.get(
                    "release_quat",
                    traj.get("last_target_quat"),
                )
                target_quat = (
                    None
                    if release_quat is None
                    else np.asarray(release_quat, dtype=np.float32).copy()
                )
            gripper = float(args.gripper_open)
            base_xy = np.asarray(base_release, dtype=np.float32).copy()
            yaw = yaw_stop
            # Reuse the existing phase label: this is a physical open hold
            # after release, with no door force and no base translation.
            phase = "open_hold"
        elif step < pass_end:
            pass_t = smoothstep(
                (step - post_release_coast_end + 1)
                / max(1, int(args.pass_through_steps))
            )
            if "offset_pass_curve_a" not in traj:
                pass_start = np.asarray(base_release, dtype=np.float32).copy()
                pass_center = np.asarray(base_push, dtype=np.float32).copy()
                pass_final = np.asarray(base_traverse, dtype=np.float32).copy()
                forward = np.asarray(
                    [math.cos(float(yaw_stop)), math.sin(float(yaw_stop))],
                    dtype=np.float32,
                )
                center_control = (
                    pass_final
                    - forward
                    * float(getattr(args, "pull_offset_center_tangent_distance", 1.64))
                )
                center_control[1] = float(pass_final[1]) + (
                    float(pass_start[1]) - float(pass_final[1])
                ) * float(
                    np.clip(
                        getattr(
                            args,
                            "pull_offset_center_control_right_fraction",
                            0.45,
                        ),
                        0.0,
                        1.0,
                ...
                center_control[1] += float(
                    getattr(
                        args,
                        "pull_offset_center_control_lateral_offset",
                        -0.20,
                    )
                )
                curve_a = (
                    ...
                    pass_start
                    + forward
                    * float(getattr(args, "pull_offset_entry_tangent_distance", 0.45)),
                    center_control,
                    pass_final,
                )
                traj["offset_pass_curve_a"] = tuple(
                    np.asarray(point, dtype=np.float32).copy() for point in curve_a
                )
                print(
                    f"[A2WPullOffset] straight-exit pass curve: "
                    f"start={np.round(pass_start, 3).tolist()} "
                    f"post_door_center={np.round(pass_center, 3).tolist()} "
                    f"final={np.round(pass_final, 3).tolist()} "
                    f"final_yaw_deg={math.degrees(float(yaw_stop)):.1f}",
                    flush=True,
                )

            curve = traj["offset_pass_curve_a"]
            base_xy = cubic_bezier_point(*curve, pass_t)
            tangent = cubic_bezier_tangent(*curve, pass_t)
            tangent_norm = float(np.linalg.norm(tangent))
            yaw = (
                math.atan2(float(tangent[1]), float(tangent[0]))
                if tangent_norm > 1.0e-7
                else float(traj.get("yaw", yaw_stop))
            )
...
def run_demo(
    ...
    yaw_start, heading, base_start, base_stop = compute_pull_offset_base_walk_targets(args, door)
    dc.configure_dynamic_walk_steps(args, base_start, base_stop)
    base_push, base_traverse = compute_pull_offset_base_targets(args, door, base_stop, heading)
    yaw_push = yaw_start + move_to_approach_yaw_delta(args) + args.push_base_yaw_delta
    yaw_traverse = yaw_start
...
def initialize_recovery_candidate(gym, sim, st, entry, dof_names, raw_root, dt):
    ...
    st.base_push, st.base_traverse = compute_pull_offset_base_targets(
        st.args,
        st.door,
        st.base_stop,
        heading,
