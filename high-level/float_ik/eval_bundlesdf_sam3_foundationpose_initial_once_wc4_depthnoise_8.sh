...
ROOT="${1:-high-level/logs/foundationpose/wc4_bundlesdf_no_gt_sam3_fp_first_valid_once_depthnoise_8_seed615455575}"
...
# Two environments per process is the stable configuration for the three-model
# Isaac Gym + SAM3 + FoundationPose stack on this workstation.
for offset in 0 2 4 6; do
  ...
  echo "===== One-shot pose with depth noise: virtual envs ${offset}..$((offset + 1)) ====="
  conda run --no-capture-output -n b1z1 python \
    ...
    --foundationpose_virtual_num_envs 8 \
...
aggregate = {
    ...
    "seed": 615455575,
    "virtual_env_indices": list(range(8)),
    "depth_noise": {
        "enabled": True,
        "env_probability": 1.0,
        "gaussian_blur": False,
...
print(f"INITIAL_ONCE_DEPTHNOISE_SUCCESS {success}/{trials} = {success / max(1, trials):.6f}")
