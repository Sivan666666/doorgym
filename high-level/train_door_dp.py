...
class DoorDPSequenceDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for idx in obs_ids:
            ...
            masks.append(_as_chw_u8(_field(frame, "observation.images.wrist_handle_mask")))
            depths.append(_as_chw_u8(_field(frame, "observation.images.wrist_masked_depth")))
        actions = []
...
def normalize_batch(batch, stats, device):
    ...
    return state, batch["mask"].to(device), batch["masked_depth"].to(device), action
...
def main():
    ...
    while step < args.steps:
        for batch in loader:
            state, mask, masked_depth, action = normalize_batch(batch, stats, device)
            ...
            pred = model(noisy_action, timesteps, state, mask, masked_depth)
