...
class DoorDiffusionPolicy(nn.Module):
    def __init__(
        ...
        per_obs_dim = state_dim + image_feature_dim * 2
    ...
    def encode_obs(self, state, mask, masked_depth):
        b, t = state.shape[:2]
        ...
        obs_feat = torch.cat([state, mask_feat, depth_feat], dim=-1)
        return self.obs_encoder(obs_feat).reshape(b, -1)

    def forward(self, noisy_action, timesteps, state, mask, masked_depth):
        obs_cond = self.encode_obs(state, mask, masked_depth)
